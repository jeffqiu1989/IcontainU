...
`volumes`（命名卷与 bind）、`networks`、`depends_on`（仅启动顺序）、`container_name`、`user`，
以及顶层 `networks:` / `volumes:`。不支持的字段（`build:`、`healthcheck:`、`restart:`、
`${VAR}` 插值、`profiles`、`secrets` 等）以警告横幅提示，不会静默丢弃。
...
`/host:/data[:ro]`，含相对路径 `./`）、`networks`、`depends_on`（仅启动顺序；`condition:` 忽略）、
`container_name`、`user`，以及顶层 `networks:` / `volumes:`。

**不支持**（以警告横幅提示，绝不静默丢弃）：`build:`、`healthcheck:`、
`depends_on: condition: service_healthy`、`restart:`、`deploy.replicas` / scale、
